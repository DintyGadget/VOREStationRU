//Pilot

/obj/item/clothing/under/rank/pilot1
	name = "\improper NanoTrasen flight suit"
	desc = "Серо-синий лётный костюм NanoTrasen. Теплый и практичный, очень удобный."
	icon_state = "pilot1"
	worn_state = "pilot1"
	item_icons = list(slot_w_uniform_str = 'maps/southern_cross/icons/mob/sc_under.dmi')
	icon = 'maps/southern_cross/icons/obj/sc_uniforms.dmi'
	sprite_sheets = list(
			"Тешари" = 'maps/southern_cross/icons/mob/species/teshari/sc_uniform.dmi'
			)
	starting_accessories = list(/obj/item/clothing/accessory/storage/webbing/pilot1)

/obj/item/clothing/under/rank/pilot2
	name = "летный костюм NanoTrasen"
	desc = "Темно-синий лётный костюм NanoTrasen. Теплый и практичный, с парой заплаток."
	icon_state = "pilot2"
	worn_state = "pilot2"
	rolled_down = 0
	rolled_sleeves = 0
	item_icons = list(slot_w_uniform_str = 'maps/southern_cross/icons/mob/sc_under.dmi')
	icon = 'maps/southern_cross/icons/obj/sc_uniforms.dmi'
	sprite_sheets = list(
			"Тешари" = 'maps/southern_cross/icons/mob/species/teshari/sc_uniform.dmi'
			)
	starting_accessories = list(/obj/item/clothing/accessory/storage/webbing/pilot2)