...
/datum/controller/subsystem/transcore/proc/notify(var/name, var/repeated = FALSE)
	ASSERT(name)
	if(repeated)
		global_announcer.autosay("This is a repeat notification that [name] is past-due for a mind backup.", "TransCore Oversight", "Медицинский отдел")
	else
		global_announcer.autosay("[name] is past-due for a mind backup.", "TransCore Oversight", "Медицинский отдел")
...
/datum/controller/subsystem/transcore/proc/core_dump(var/obj/item/weapon/disk/transcore/disk)
	ASSERT(disk)
	global_announcer.autosay("An emergency core dump has been initiated!", "TransCore Oversight", "Командование")
	global_announcer.autosay("An emergency core dump has been initiated!", "TransCore Oversight", "Медицинский отдел")
