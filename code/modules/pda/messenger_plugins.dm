...
/datum/data/pda/messenger_plugin/virus/clown/user_act(mob/user as mob, obj/item/device/pda/P)
	. = ..(user, P)
	if(.)
		user.show_message("<span class='notice'>Вирус отправлен!</span>", 1)
...
/datum/data/pda/messenger_plugin/virus/detonate/user_act(mob/user as mob, obj/item/device/pda/P)
	...
	if(.)
		...
		if(!P.detonate || P.hidden_uplink)
			user.show_message("<span class='warning'>Похоже, что ПДА цели не реагирует на команду детонации.</span>", 1)
			pda.cartridge.charges++
		else if(prob(difficulty * 12))
			user.show_message("<span class='warning'>На вашем [pda] мигает ошибка.</span>", 1)
		else if(prob(difficulty * 3))
			user.show_message("<span class='danger'>Энергия возвращается в ваш [pda]!</span>", 1)
			pda.close(user)
			pda.explode()
			log_admin("[key_name(user)] пытался взорвать [P] картриджем Detomatix, но потерпел неудачу, взорвав себя")
...
/datum/data/pda/messenger_plugin/virus/frame/user_act(mob/user, obj/item/device/pda/P)
	...
	if(.)
		...
		user.show_message("<span class='notice'>Вирус отправлен! Код разблокировки для цели: [lock_code]</span>")
		if(!P.hidden_uplink)
