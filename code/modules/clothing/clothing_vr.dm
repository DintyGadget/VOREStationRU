...
/obj/item/clothing/shoes/attackby(var/obj/item/I, var/mob/user)
	if(istype(I,/obj/item/weapon/holder/micro))
		...
		for(var/mob/M in src)
			full++
		if(full >= 2)
			to_chat(user, "<span class='warning'>В [ru_getcase(src, "acase")] больше никто не поместится!</span>")
		else
			var/obj/item/weapon/holder/micro/holder = I
			if(holder.held_mob && holder.held_mob in holder)
				to_chat(holder.held_mob, "<span class='warning'>[user] запихивает Вас в [ru_getcase(src, "acase")]!</span>")
				holder.held_mob.forceMove(src)
				to_chat(user, "<span class='notice'>Вы запихиваете [holder.held_mob] в [ru_getcase(src, "acase")]!</span>")
	else
		..()
...
/obj/item/clothing/shoes/attack_self(var/mob/user)
	for(var/mob/M in src)
		...
		to_chat(M, "<span class='warning'>[user] вытряхивает Вас из [ru_getcase(src, "gcase")]!</span>")
		to_chat(user, "<span class='notice'>Вы вытряхиваете [M] из [ru_getcase(src, "gcase")]!</span>")
...
/obj/item/clothing/shoes/container_resist(mob/living/micro)
	...
	if(!istype(macro))
		to_chat(micro, "<span class='notice'>Вы начинаете выбираться из [ru_getcase(src, "gcase")]!</span>")
		if(do_after(micro, 50, src))
			to_chat(micro, "<span class='notice'>Вы выбираетесь из [ru_getcase(src, "gcase")]!</span>")
			micro.forceMove(loc)
		return

	var/escape_message_micro = "Вы начинаете выбираться из [ru_getcase(src, "gcase")]!"
	var/escape_message_macro = "Кто-то пытается выбраться из Ваших [ru_getcase(src, "gcase")]!"
	var/escape_time = 60
	...
	to_chat(micro, "<span class='notice'>Вы смогли выбраться из [ru_getcase(src, "gcase")]!</span>")
	to_chat(macro, "<span class='danger'>Кто-то сбежал из Ваших [ru_getcase(src, "gcase")]!</span>")
...
/obj/item/clothing/relaymove(var/mob/living/user,var/direction)
	...
	if(ishuman(src.loc))
		...
		if(H.shoes == src)
			to_chat(H, "<font color='red'>Крохотное туловище [user] прижато к Вам внутри [ru_getcase(src, "gcase")], дёргаясь!</font>")
			to_chat(user, "<font color='red'>Ваше тело прижато к [H]! Не повезло Вам с размером!</font>")
		else
			to_chat(H, "<font color='red'>Туловище [user] дрыгается внутри [ru_getcase(src, "gcase")]!</font>")
			to_chat(user, "<font color='red'>Вы пытаетесь дрыгаться внутри [ru_getcase(src, "gcase")], безуспешно.</font>")
	else
