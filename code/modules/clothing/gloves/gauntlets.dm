...
/obj/item/clothing/gloves/gauntlets/mob_can_equip(mob/user, slot, disable_warning = FALSE)
	...
	if(H.gloves)
		gloves = H.gloves
		if(!istype(gloves))
			to_chat(user, "Вы не можете надеть [ru_getcase(src, "acase")], так как на Вас [ru_getcase(H.gloves. "ncase")].")
			gloves = null
			return 0
	...
	if(!..())
		if(gloves)
			...
			return 0
	if(gloves)
		to_chat(user, "Вы надеваете [ru_getcase(src, "acase")] поверх перчаток.")
	wearer = H
	return 1
