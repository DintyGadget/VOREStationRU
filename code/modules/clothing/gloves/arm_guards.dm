	...
	if(..()) //This will only run if no other problems occured when equiping.
		if(H.wear_suit)
			if(H.wear_suit.body_parts_covered & ARMS)
				to_chat(H, "<span class='warning'>Вы не можете надеть [src], так как на Вас [H.wear_suit].</span>")
				return 0
			for(var/obj/item/clothing/accessory/A in H.wear_suit)
				if(A.body_parts_covered & ARMS)
					to_chat(H, "<span class='warning'>Вы не можете надеть [src], так как на Вас [A].</span>")
					return 0
		return 1
