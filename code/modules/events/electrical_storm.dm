...
/datum/event/electrical_storm/announce()
	...
	switch(severity)
		if(EVENT_LEVEL_MUNDANE)
			command_announcement.Announce("Небольшая электрическая буря была обнаружена поблизости [location_name()]. Обратите внимание на возможные электрические разряды.", "[location_name()] Sensor Array")
		if(EVENT_LEVEL_MODERATE)
			command_announcement.Announce("[location_name()] вот-вот пройдет через грозу. Пожалуйста, сохраните чувствительное электрическое оборудование, пока не пройдет шторм.", "[location_name()] Sensor Array")
		if(EVENT_LEVEL_MAJOR)
			command_announcement.Announce("Предупреждение. Поблизости [location_name()] была обнаружена сильная гроза. Рекомендуется немедленно обезопасить чувствительное электрическое оборудование, пока ураган не пройдет.", "[location_name()] Sensor Array")
