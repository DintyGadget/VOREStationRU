/datum/event/prison_break
	...
	var/eventDept = "Служба безопасности"			//Department name in announcement
...
/datum/event/prison_break/announce()
	if(areas && areas.len > 0)
		command_announcement.Announce("[pick("Gr3y.T1d3 virus","Malignant trojan")] detected in [station_name()] [(eventDept == "Служба безопасности")? "imprisonment":"containment"] subroutines. Secure any compromised areas immediately. Station AI involvement is recommended.", "[eventDept] Alert")
...
/datum/event/prison_break/start()
	...
	if(areas && areas.len > 0)
		...
		for(var/obj/machinery/message_server/MS in machines)
			MS.send_rc_message("Инженерный отдел", my_department, rc_message, "", "", 2)
