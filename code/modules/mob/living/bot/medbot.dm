...
/mob/living/bot/medbot/UnarmedAttack(var/mob/living/carbon/human/H)
	...
	if(declare_treatment)
		...
		global_announcer.autosay("[src] is treating <b>[H]</b> in <b>[location]</b>", "[src]", "Медицинский отдел")
	busy = 1
	update_icons()
	if(do_mob(src, H, 30))
		if(t == 1)
...
/mob/living/bot/medbot/proc/handle_panic()
	...
	switch(tipped_status)
		...
		if(MEDBOT_PANIC_END)
			global_announcer.autosay("PSYCH ALERT: Crewmember [tipper_name] recorded displaying antisocial tendencies torturing bots in [get_area(src)]. Please schedule psych evaluation.", "[src]", "Медицинский отдел")
