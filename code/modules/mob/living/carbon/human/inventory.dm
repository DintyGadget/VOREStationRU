...
/mob/living/carbon/human/verb/quick_equip()
	...
	if(ishuman(src))
		...
		if(!I)
			to_chat(H, "<span class='notice'>Вы не держите ничего, что можно надеть.</span>")
			return
		if(H.equip_to_appropriate_slot(I))
			...
			to_chat(H, "<font color='red'>Вы не можете это надеть.</font>")
...
/mob/living/carbon/human/equip_to_slot(obj/item/W as obj, slot)
	...
	switch(slot)
		...
		if(slot_tie)
			...
			to_chat(src, "<font color='red'>Вы пытаетесь это надеть на неподдерживаемый слот инвентаря. Как Вы вообще умудрились? Не надо так...</font>")
			return
