var/list/department_radio_keys = list(
	  ...
	  ":c" = "Командование",		".c" = "Командование",
	  ":n" = "Научный отдел",		".n" = "Научный отдел",
	  ":m" = "Медицинский отдел",		".m" = "Медицинский отдел",
	  ":e" = "Инженерный отдел", ".e" = "Инженерный отдел",
	  ":k" = "Response Team",	".k" = "Response Team",	//TFF 11/3/20 - Add Response Team to channels usable rather than resorting to :H or such.,
	  ":s" = "Служба безопасности",	".s" = "Служба безопасности",
	  ":w" = "whisper",		".w" = "whisper",
	  ":t" = "Mercenary",	".t" = "Mercenary",
	  ":x" = "Raider",		".x" = "Raider",
	  ":u" = "Снабжение",		".u" = "Снабжение",
	  ":v" = "Сервис",		".v" = "Сервис",
	  ...
	  ":н" = "intercom",	".н" = "intercom",
	  ":д" = "department",	".д" = "department",
	  ":к" = "Командование",		".к" = "Командование",
	  ":у" = "Научный отдел",		".у" = "Научный отдел",
	  ":м" = "Медицинский отдел",		".м" = "Медицинский отдел",
	  ":и" = "Инженерный отдел",	".и" = "Инженерный отдел",
	  ":б" = "Служба безопасности",	".б" = "Служба безопасности",
	  ":ш" = "whisper",		".ш" = "whisper",
	  ":а" = "Mercenary",	".а" = "Mercenary",
	  ":с" = "Снабжение",		".с" = "Снабжение",
)
...
/mob/living/proc/handle_speech_problems(var/list/message_data)
	...
	for(var/datum/multilingual_say_piece/S in message_pieces)
		...
		if((HULK in mutations) && health >= 25 && length(S.message))
			...
			verb = pick("орёт","вопит","кричит")
			whispering = 0
			. = 1
		if(slurring)
			S.message = slur(S.message)
			verb = pick("бормочет","бубнит")
			. = 1
		if(stuttering)
			S.message = stutter(S.message)
			verb = pick("запинается","еле выговаривает")
			. = 1
...
/mob/living/say(var/message, var/whispering = 0)
	...
	if(client)
		if(message)
			...
			if((client.prefs.muted & MUTE_IC) || say_disabled)
				to_chat(src, "<span class='warning'>Вы не можете говорить IC (мут).</span>")
				return
	...
	if(is_muzzled())
		to_chat(src, "<span class='danger'>Ваш рот заткнут!</span>")
		return
	...
	if(!whispering) //Just doing normal 'say' (for now, may change below)
		...
		w_not_heard = "[first_piece.speaking.speech_verb] что-то [w_adverb]"
	...
	if(used_radios.len)
		...
		if(!first_piece.speaking || !(first_piece.speaking.flags & NO_TALK_MSG))
			msg = "<span class='notice'>[src] [ru_g_mob(src, "говор", "ит", "ит", "ит", "ят")] в [used_radios[1]]</span>"
	...
	if(message_mode)
		message = "([message_mode == "headset" ? "Общий" : capitalize(message_mode)]) [message]" //Adds radio keys used if available
	if(whispering)
