...
/datum/lore/atc_controller/proc/shift_ending(var/evac = 0)
	msg("[using_map.shuttle_name], это Контроль [using_map.dock_name], вам разрешено выполнить обычную передачу из [using_map.station_name] d [using_map.dock_name].")
	sleep(5 SECONDS)
	msg("[using_map.shuttle_name] отправляется из [using_map.dock_name] в [using_map.station_name] по обычному пересадочному маршруту. Расчетное время до прибытия: 10 минут.","[using_map.shuttle_name]")
...
/datum/lore/atc_controller/proc/random_convo()
	...
	switch(chatter_type)
		//mayday call
		if("emerg")
			var/problem = pick("У нас есть пробоины в корпусе на нескольких палубах","У нас на борту есть неизвестные враждебные формы жизни","Наш основной двигатель выходит из строя","У нас есть астероиды, ударяющие по корпусу","Мы испытываем полную потерю мощности двигателя","Вражеские корабли быстро приближаются","В кабине дым","У нас есть неопознанные жильцы","Наша система жизнеобеспечения отказала")
			msg("+СОС, СОС, СОС!+ Это [combined_first_name] объявляем чрезвычайную ситуацию! [problem]!","[prefix] [shipname]")
			sleep(5 SECONDS)
			msg("[combined_first_name], это Контроль [using_map.dock_name], повторите. Переключитесь на канал аварийного реагирования [ertchannel].")
			sleep(5 SECONDS)
			msg("Принято Контроль [using_map.dock_name], переключаемся.","[prefix] [shipname]")
		//Control scan event: soft outcome
		if("policescan")
			var/confirm = pick("Поняли","Принято","Подтверждаю")
			var/complain = pick("Надеюсь, это не займет слишком много времени.","Мы можем поторопиться с этим?","Только быстрее.","Лучше бы это не заняло слишком много времени.","Это действительно необходимо?")
			var/completed = pick("Вы можете продолжать движение.","Все хорошо, продолжайте движение.","Чисто, двигайтесь дальше.","Прошу прощения за задержку, все чисто.","Переключитесь на канал [sdfchannel] и ждите дальнейших инструкций.")
			msg("[combined_first_name], это Контроль [using_map.dock_name], [pick("ваш корабль","ваше судно","ваш звездолет")] был помечен для плановой проверки. Удерживайте позицию и приготовьтесь к сканированию.")
			sleep(5 SECONDS)
			msg("[confirm] Контроль [using_map.dock_name], останавливаем движение.","[prefix] [shipname]")
			sleep(5 SECONDS)
			msg("Мы ценим вашу уступчивость, [combined_first_name]. Начинаем сканирование.")
			sleep(10 SECONDS)
			msg(complain,"[prefix] [shipname]")
			sleep(15 SECONDS)
			msg("[combined_first_name], это Контроль [using_map.dock_name]. Сканирование завершено. [completed]")
		//Control scan event: hard outcome
		if("policeflee")
			var/uhoh = pick("Ничего не поделаешь, шеф, нам есть куда слетать.","Извини, но нам нужно кое-где побывать.","Этого не будет.","Блядь, черт возьми, кто сдал нас на этот раз?!","Вы никогда не возьмете меня живым!","Эй, у меня есть маскировочное устройство! Ты меня не видишь!","Мне нужно будет попросить вернуть деньги за этот стелс двигатель...","Боюсь, я не смогу этого сделать, Контроль.","Сраный |ад|.","Блядь!","Это не тот корабль, который вы ищете.","Что ж. Вышло неловко.","Ам..ум.","Я сдаюсь!")
			msg("[pick("Неизвестный корабль","Неизвестное судно","Неизвестный звездолет")], это Контроль [using_map.dock_name], назовите себя и пройдите полную проверку. Полет без активного транспондера является нарушением правил системы.")
			sleep(5 SECONDS)
			msg("[uhoh]","[shipname]")
			sleep(5 SECONDS)
			msg("Это Контроль Защиты [using_map.starsys_name] обращаюсь ко всем местным службам: вектор для перехвата и задержания [combined_first_name]. Все вышло из-под контроля.","Контроль Защиты [using_map.starsys_name]")
		//SDF scan event: soft outcome
		if("policeshipscan")
			var/confirm = pick("Поняли","Принято","Подтверждаю")
			var/complain = pick("Надеюсь, это не займет слишком много времени.","Мы можем поторопиться с этим?","Только быстрее.","Лучше бы это не заняло слишком много времени.","Это действительно необходимо?")
			var/completed = pick("Вы можете продолжать движение.","Все хорошо, продолжайте движение.","Чисто, двигайтесь дальше.","Прошу прощения за задержку, все чисто.","Переключитесь на канал [sdfchannel] и ждите дальнейших инструкций.")
			msg("[combined_second_name], это [combined_first_name], [pick("ваш корабль","ваше судно","ваш звездолет")] был помечен для плановой проверки. Удерживайте позицию и приготовьтесь к сканированию.","[prefix] [shipname]")
			sleep(5 SECONDS)
			msg("[confirm] [combined_first_name], останавливаем движение.","[secondprefix] [secondshipname]")
			sleep(5 SECONDS)
			msg("Мы ценим вашу уступчивость, [combined_second_name]. Начинаем сканирование.","[prefix] [shipname]")
			sleep(10 SECONDS)
			msg(complain,"[secondprefix] [secondshipname]")
			sleep(15 SECONDS)
			msg("[combined_second_name], это [combined_first_name]. Сканирование завершено. [completed]","[prefix] [shipname]")
		//SDF scan event: hard outcome
		if("policeshipflee")
			var/uhoh = pick("Ничего не поделаешь, шеф, нам есть куда слетать.","Извини, но нам нужно кое-где побывать.","Этого не будет.","Блядь, черт возьми, кто сдал нас на этот раз?!","Вы никогда не возьмете меня живым!","Эй, у меня есть маскировочное устройство! Ты меня не видишь!","Мне нужно будет попросить вернуть деньги за этот стелс двигатель...","Боюсь, я не смогу этого сделать, Контроль.","Сраный |ад|.","Блядь!","Это не тот корабль, который вы ищете.","Что ж. Вышло неловко.","Ам..ум.","Я сдаюсь!")
			msg("[pick("Неизвестный корабль","Неизвестное судно","Неизвестный звездолет")], это [combined_second_name], назовите себя и пройдите полную проверку. Полет без активного транспондера является нарушением правил системы.","[secondprefix] [secondshipname]")
			sleep(5 SECONDS)
			msg("[uhoh]","[shipname]")
			sleep(5 SECONDS)
			msg("Контроль Защиты [using_map.starsys_name], это [combined_second_name], у нас тут ситуация, требуется консультация.","[secondprefix] [secondshipname]")
			sleep(5 SECONDS)
			msg("Копии Управления Обороны, [combined_second_name], подкрепление в пути. Переключите дальнейшую связь на зашифрованный канал [sdfchannel].","[using_map.starsys_name] Defense Control")
