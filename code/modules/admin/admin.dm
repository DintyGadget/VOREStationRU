	...
	if(channel) //They picked a channel
		...
		if(sender) //They put a sender
			...
			if(message) //They put a message
				message = sanitize(message, 500, extra = 0)
				global_announcer.autosay("[message]", "[sender]", "[channel == "Общий" ? null : channel]") //Common is a weird case, as it's not a "channel", it's just talking into a radio without a channel set.
...
/datum/admins/proc/intercom_convo()
	...
	if(LAZYLEN(decomposed))
		for(var/i = 1; i < decomposed.len; i++)
			...
			global_announcer.autosay("[this_message]", "[this_sender]", "[channel == "Общий" ? null : channel]") //Common is a weird case, as it's not a "channel", it's just talking into a radio without a channel set.
...
/datum/admins/proc/sendFax()
	set category = "РЎРїРµС†. РєРѕРјР°РЅРґС‹"
	set name = "Send Fax"
