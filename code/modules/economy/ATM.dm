...
/obj/machinery/atm/emag_act(var/remaining_charges, var/mob/user)
	...
	var/response = pick("Инициирование выдачи. Хорошего дня!", "КРИТИЧЕСКАЯ ОШИБКА: Activating cash chamber panic siphon.","Пин-код принят! Очистка баланса.", "Джекпот!")
				...
				if(authenticated_account.suspended)
					...
					switch(view_screen)
						if(CHANGE_SECURITY_LEVEL)
							...
							dat += "[text]<hr><br>"
							dat += "<A href='?src=\ref[src];choice=view_screen;view_screen=0'>Назад</a>"
				...
				if(authenticated_account)
					...
					if(transfer_amount <= 0)
						...
						if(charge_to_account(target_account_number, authenticated_account.owner_name, transfer_purpose, machine_id, transfer_amount))
							...
							var/datum/transaction/T = new()
							T.target_name = "Аккаунт #[target_account_number]"
